export default class AdeleAnnotationApiAdapter {
    /** */
    constructor(docId, canvasId, callbacks) {
    ...
    async create(annotation) {
      return fetch(`${this.endpointUrl}/create`, {
        body: JSON.stringify(AdeleAnnotationApiAdapter.createV2Anno(annotation)),
        headers: {
    ...
    async all() {
      const annos = await this.callbacks.fetchAll(this.docId, this.canvasId)
    ...
    createAnnotationPage(v2annos) {
      if (Array.isArray(v2annos)) {
        const v3annos = v2annos.map((a) => AdeleAnnotationApiAdapter.createV3Anno(a));
        return {
          id: this.canvasId,
          items: v3annos,
          type: 'AnnotationPage',
